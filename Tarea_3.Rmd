...
Cargemos los datos
```{r }
load('~/Desktop/ALEXA/Maestria/Segundo Semestre/Economia Computacional/tarea_3/universo.Rdata')
...
tau_min(outcome_var=universo$tasa_trans,N=length(universo$total_purchases),share_control=n_control/(n_control+n_tratamiento),n_groups = 2)
```


#El efecto mínimo detectable es 0.0689% dad las 90107 observaciones asignadas al grupo control. Esto quiere decir que, a partir de una diferencia de 0.0689% en la tasa de transaccionalidad podría ser significativo el efecto de tratamiento dado el tamaño de la población 104191,una proporción de 13.5% de los inidviduos asignados al grupo de tratamiento, el poder estadístico de 0.8 y una significacncia de 0.05.
...
```{r}
tau_min(outcome_var=universo$total_purchases,N=length(universo$total_purchases),share_control=n_control/(n_control+n_tratamiento),n_groups = 2)
```

### 4 (2ptos) Qué variables crees que puedan estar más correlacionadas con el impacto? Justifica tu respuesta y elige un set

#device value: Esta variable está correlacionada con el impacto ya que posiblemente tenga un mayor impacto en alguien que tiene un valor
#de dispositivo bajo ya que esta ayuda sería de mayor valor. Mientras que alguien que ya ha gastado mucho es menos probable que aumente 
#ya que relativo al valor que gastan el monto de los cupones es muy bajo. 

```{r}
kable(universo%>%group_by(device_value)%>%summarise(media=mean(total_purchases)))
```


#Edad: Es más probable que los individuos jóvenes tengan menores ingresos, por lo que un cupón es más 
#probable que aumente sus compras en la aplicación. Por el otro lado, es más probable que la gente 
#mayor se olvide de la aplicación por lo que un mensaje informativo podría aumentar sus compras. 

```{r}
edad<-universo%>%group_by(age)%>%summarise(media=mean(total_purchases))

plot(x=edad$age,y=edad$media)
```

#Gender: En la tabla a continuación se puede observar que los hombres gastan menos que las mujeres. Por lo tanto, darle
#un cupón o un recordatorio a los hombres podría tener mayor impacto. 


```{r}
kable(universo%>%group_by(gender_F)%>%summarise(media=mean(total_purchases)))
```
...
kable(estratos_sum_missfit)

estratos_sum_missfit_2<-estratos$summary_strata
kable(estratos_sum_missfit_2)
...
#Sí parece haber balance ya que las medias de tratamiento y control son iguales para todas las categorias utilizadas y el valor-p siempre es igual a 1 por lo que no se puede rechazar que la diferencia de las medias sea diferente de 0. 
